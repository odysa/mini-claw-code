...
Before tackling the main loop, we need a helper that takes a slice of `ToolCall`s from the LLM and produces results. This is `execute_tools`:
...
6. **`run_with_history`** -- Same loop as `chat` but emit `AgentEvent`s through a channel. Handle errors as events instead of `?`.
