...
> **Tests to run:** `cargo test -p mini-claw-code-starter test_mock_` (MockProvider), `cargo test -p mini-claw-code-starter test_openrouter_` (OpenRouterProvider), `cargo test -p mini-claw-code-starter streaming` (streaming)
...
By the end, the mock provider tests in `test_mock_`, the OpenRouter tests in `test_openrouter_`, and the streaming tests in `streaming` should pass.

```bash
cargo test -p mini-claw-code-starter test_mock_
cargo test -p mini-claw-code-starter test_openrouter_
cargo test -p mini-claw-code-starter streaming
```
...
```rust
#[tokio::test]
async fn test_read_mock_returns_text() {
...
The `test_read_accumulator_text` test feeds two `TextDelta` events and verifies the concatenated result. The `test_read_accumulator_tool_call` test feeds a `ToolCallStart` followed by two `ToolCallDelta` fragments (`{"path":` and `"test.txt"}`) and verifies they are reassembled into a valid `ToolCall` with parsed JSON arguments.
...
```bash
cargo test -p mini-claw-code-starter test_mock_   # MockProvider tests
cargo test -p mini-claw-code-starter test_openrouter_   # OpenRouterProvider tests
cargo test -p mini-claw-code-starter streaming  # Streaming tests
```

### What the tests verify

**MockProvider tests (test_mock_):**
...
**OpenRouterProvider tests (test_openrouter_):**

- **`test_openrouter_convert_messages`** -- verifies that internal `Message` variants are converted to the correct OpenAI API format
- **`test_openrouter_convert_tools`** -- verifies that `ToolDefinition` values are wrapped in the OpenAI function-calling envelope
