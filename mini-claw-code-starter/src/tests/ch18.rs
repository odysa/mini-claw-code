...
#[test]
fn test_ch18_path_within_allowed() {
...
#[test]
fn test_ch18_path_outside_allowed() {
    let dir = tempfile::tempdir().unwrap();
    let validator = PathValidator::new(dir.path());
...
#[test]
fn test_ch18_path_traversal_blocked() {
    let dir = tempfile::tempdir().unwrap();
...
#[test]
fn test_ch18_path_new_file_in_allowed() {
    let dir = tempfile::tempdir().unwrap();
    let validator = PathValidator::new(dir.path());
...
#[test]
fn test_ch18_safety_check_read_tool() {
    let dir = tempfile::tempdir().unwrap();
...
#[test]
fn test_ch18_safety_check_ignores_bash() {
    let dir = tempfile::tempdir().unwrap();
    let validator = PathValidator::new(dir.path());
...
#[test]
fn test_ch18_command_filter_blocks_rm_rf() {
...
#[test]
fn test_ch18_command_filter_blocks_sudo() {
    let filter = CommandFilter::default_filters();
    assert!(filter.is_blocked("sudo rm file").is_some());
}

#[test]
fn test_ch18_command_filter_allows_safe() {
    let filter = CommandFilter::default_filters();
...
#[test]
fn test_ch18_command_filter_safety_check() {
    let filter = CommandFilter::default_filters();
    assert!(
        filter
...
#[test]
fn test_ch18_protected_file_blocks_env() {
...
#[test]
fn test_ch18_protected_file_allows_normal() {
...
#[tokio::test]
async fn test_ch18_wrapper_blocks_on_check_failure() {
...
#[tokio::test]
async fn test_ch18_wrapper_allows_valid_call() {
    use crate::tools::ReadTool;
...
#[test]
fn test_ch18_custom_blocked_commands() {
